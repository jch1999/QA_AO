                                            ...
                                            bool bReplicateEndAbility, bool bWasCancelled)
{
	...
	if (ASC)
	{
		...
		if (ActorInfo->IsNetAuthority())
		{
			...
			// 플레이어가 스프린트 키를 자발적으로 해제했을 때(bWasCancelled=false),
			// 스태미나가 Threshold 미만이면 Lockout 태그를 추가한다.
			// (스태미나 0% 강제 종료 시에는 HandleStaminaLockout에서 이미 처리됨)
			if (!bWasCancelled)
			{
				const UAO_PlayerCharacter_AttributeSet* AttributeSet = ASC->GetSet<UAO_PlayerCharacter_AttributeSet>();
				if (AttributeSet)
				{
					const float CurrentStamina = AttributeSet->GetStamina();
					const float Threshold = AttributeSet->GetMaxStamina() * AttributeSet->StaminaLockoutPercent;
					const FGameplayTag LockoutTag = FGameplayTag::RequestGameplayTag(FName("Status.Lockout.Stamina"));

					if (CurrentStamina < Threshold && !ASC->HasMatchingGameplayTag(LockoutTag))
					{
						ASC->AddLooseGameplayTag(LockoutTag);
						AOFUNC_LOG(LogKSH, Log, TEXT("Stamina lockout applied on sprint release (Stamina: %.1f / Threshold: %.1f)"),
							CurrentStamina, Threshold);
					}
				}
			}
		}
